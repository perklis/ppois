...
class HeadPhysician(Employee):
    ...
    def request_patient_report(self, doctor: Doctor, medical_card_number: str):
        """Получить выписку пациента по номеру медкарты у конкретного врача"""
        if doctor not in self.__doctors:
            raise PermissionError(f"Doctor {doctor.name} is not in your staff")
        ...
        raise ValueError("Пациент с таким номером карты не найден у данного врача")
    ...
    def give_bonus_to_employees(
        ...
        """Выдать премию сотрудникам с зарплатой ниже threshold"""
    ...
    def request_security_report(self, security_department: SecurityDepartment) -> str:
        """Запросить все отчёты об инцидентах у отдела охраны"""
    ...
    def request_legal_report(self, lawyer: Lawyer):
        """Запрашивает у юриста единый отчёт (дела + жалобы)"""
        if not isinstance(lawyer, Lawyer):
            raise TypeError("Expected Lawyer instance")
        print(f"Head Physician {self.name} requested a legal report from {lawyer.name}")
