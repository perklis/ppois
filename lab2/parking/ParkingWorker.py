...
from exceptions import CarNotFoundError, NoFreeSpotError
...
class ParkingWorker(Employee):
    ...
    def remove_car(self, spot_number: int):
        if spot_number not in self._parking_lot.get_occupied_spots():
    ...
    def get_free_spots(self):
        ...
        if not free:
            raise NoFreeSpotError(f"{self.name} reports free spots: {free}")
        return free
