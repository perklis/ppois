...
class SecurityDepartment:
    ...
    def add_guard(self, guard: SecurityGuard) -> str:
        if not isinstance(guard, SecurityGuard):
            raise InvalidGuardTypeError("Only SecurityGuard objects can be added")
        if guard in self.__guards:
            raise GuardAlreadyExistsError(
                f"{guard.name} already works in the department"
            )
    ...
    def remove_guard(self, guard: SecurityGuard) -> str:
        if guard not in self.__guards:
            raise GuardNotFound(f"{guard.name} is not found in {self.department_name}")
    ...
    def assign_shift(self, guard: SecurityGuard, shift_type: str) -> str:
        if guard not in self.__guards:
            raise GuardNotFound(f"{guard.name} is not part of {self.department_name}")
        guard.shift_type = shift_type
        return f"{guard.name} assigned to {shift_type} shift"

    def __str__(self):
        return (
            f"Security Department: {self.department_name}\n"
