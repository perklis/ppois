...
class PatientWindow(QWidget):
    def __init__(self, patient_name):
        ...
        self.book_btn.setStyleSheet(
            "background-color:#4CAF50; color:white; padding:15px; border-radius:10px;"
        )
        ...
        self.cancel_btn.setStyleSheet(
            "background-color:#f44336; color:white; padding:10px; border-radius:10px;"
        )
        ...
        self.buy_btn.setStyleSheet(
            "background-color:#9C27B0; color:white; padding:10px; border-radius:10px;"
        )
        ...
        self.back_btn.setStyleSheet(
            "background-color:#FFC107; color:black; padding:10px; border-radius:10px;"
        )
    ...
    def update_doctors(self, specialty):
        ...
        self.doctor_select.addItems(
            [name for name, spec in DOCTORS_DB.items() if spec == specialty]
        )
        ...
        if dialog.result() == QDialog.Accepted:
            ...
            if selected_item:
                ...
                QMessageBox.information(
                    self, "Успешно", f"Вы записаны к {doctor_name} на {date} в {time}"
                )
    ...
    def cancel_appointment(self):
        ...
        appointments[(date, doctor_name)] = [
            r
            for r in appointments[(date, doctor_name)]
    ...
    def buy_medicine(self):
        ...
        if bought:
            ...
            QMessageBox.information(
                self, "Ошибка", "Нет доступных лекарств по рецептам"
            )
